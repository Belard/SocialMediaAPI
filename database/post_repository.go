...
func (d *Database) CreatePost(post *models.Post) error {
	query := `INSERT INTO posts (id, user_id, content, media_ids, platforms, status, scheduled_for, created_at, updated_at)
	...
	_, err := d.DB.Exec(query, post.ID, post.UserID, post.Content, post.MediaIDs,
		platforms, post.Status, post.ScheduledFor, post.CreatedAt, post.UpdatedAt)
	return err
}

func (d *Database) UpdatePost(post *models.Post) error {
	query := `UPDATE posts SET content = $1, media_ids = $2, platforms = $3, 
			  status = $4, scheduled_for = $5, published_at = $6, updated_at = $7
			  WHERE id = $8`
	...
	_, err := d.DB.Exec(query, post.Content, post.MediaIDs, platforms,
...
func (d *Database) GetUserPosts(userID string) ([]*models.Post, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.ID, &post.UserID, &post.Content, &mediaIDs,
			&platforms, &post.Status, &post.ScheduledFor, &post.PublishedAt,
