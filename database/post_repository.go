...
func (d *Database) CreatePost(post *models.Post) error {
	...
	_, err := d.DB.Exec(query, post.ID, post.UserID, post.Content, pq.Array(post.MediaIDs),
...
func (d *Database) UpdatePost(post *models.Post) error {
	...
	_, err := d.DB.Exec(query, post.Content, pq.Array(post.MediaIDs), pq.Array(platforms),
...
func (d *Database) GetPost(id string) (*models.Post, error) {
	...
	err := d.DB.QueryRow(query, id).Scan(&post.ID, &post.UserID, &post.Content,
		pq.Array(&mediaIDs), pq.Array(&platforms), &post.Status, &post.ScheduledFor,
...
func (d *Database) GetUserPosts(userID string) ([]*models.Post, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&post.ID, &post.UserID, &post.Content, pq.Array(&mediaIDs),
